...
export class CalendarGenerator extends React.Component<
  {},
  CalendarGeneratorState
> {
  ...
  generateAction() {
    ...
    let e =
      (planet_aphelion - planet_perihelion) /
      (planet_aphelion + planet_perihelion);
  ...
  handleInputUpdate(id: string, new_value: number) {
    this.setState({
      input_values: update(this.state.input_values, {
        [id]: { $set: new_value },
      }),
    });
  }
  ...
  render() {
    return (
      <div>
        <GeneratorInput
          {...extend(this.initial_state, {
            onClick: () => this.runGeneration(),
            onChange: (id, v) => this.handleInputUpdate(id, v),
          })}
        />
        <CalendarDescription description={this.state.description} />
        {this.state.calendar_data !== null ? (
          <CalendarExample
            calendar={this.state.calendar_data.calendar}
            days_per_week={7}
            seasons={this.state.calendar_data.seasons}
            lunar_phases={this.state.calendar_data.lunar_phases}
          />
        ) : (
          false
        )}
...
export function initializeComponents() {
  ReactDOM.render(<CalendarGenerator />, document.getElementById("generator"));
}
